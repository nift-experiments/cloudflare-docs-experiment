...
**CP6 status as of 2026-09-19 (development VPS):** CP6 is split into **CP6A (ordinary documentation corpus — CERTIFIED)** and **CP6B (complete frozen-site surface + rendering correctness — IN PROGRESS)**. CP6A: all 6,882 frozen docs import with zero unknown/unresolved constructs; route verifier reports 0 missing routes; `~/assets`/`src/assets`/`public/` references rewritten; importer corpus robustness covered by 25 regression tests. **CP6B rendering architecture IMPLEMENTED:** the importer emits component/directive bodies as file-based nested Nift `@markup("md", "content/.markup/bodies/N.md")` references, so Nift renders each body's Markdown with Markup++ directly (avoiding inline `find_balanced` fragility with apostrophes, stray backticks, irregular fences). All 8,952 tracked pages build with 0 Nift HTML-validation failures; REAL rendering leakage dropped from 97 (baseline) to 18 (code-content-with-HTML edge cases + scanner false positives on unclosed `<pre>`, each individually classified). Generated families now include: changelog, glossary, directory, fields catalog, Workers AI + catalog models, learning paths, llms.txt, videos, agent-setup, synthesized WARP-release changelog posts, compatibility-flags.json, changelog RSS — raising the tracked/build surface to **8,952 pages** (development measurements ~18-27 s full / ~11-12 s no-op / peak RSS ~44-66 MB on g6-standard-1; not comparable to the Astro baseline). **Remaining CP6B work:** release-notes, dash-routes, page-build-env, pages-presets, notifications, logpush (fetched data), sitemap-index.xml families; the ~18 REAL leakage residuals; and the API-reference family (external sibling app, not generated). CP6B is not fully certified until the remaining reproducible families are generated and the leak residuals are reduced or individually documented as unavoidable. CP7 may be developed independently; CP8 full parity must not begin from an uncertified corpus.
