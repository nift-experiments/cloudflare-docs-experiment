...
**CP6 status as of 2026-09-18 (development VPS):** CP6 is split into **CP6A (ordinary documentation corpus — CERTIFIED)** and **CP6B (complete frozen-site surface + rendering correctness — PENDING)**. CP6A: all 6,882 frozen docs import with zero unknown/unresolved constructs; Nift builds all 6,883 tracked pages; route verifier reports 0 missing routes; `~/assets` references rewritten and resolve; importer corpus-driven robustness covered by 10 regression tests. CP6B requires generating the data-driven families (API reference, changelog, glossary, directory, workers-ai models, field catalog, logpush, learning paths, llms.txt, agent-setup, videos, resources, RSS/XML/JSON, etc.) from their `src/content/` collections or fetched data, expanding the expected-route manifest to the complete frozen site, and fixing rendering-correctness defects (literal `:::`/MDX/directive syntax, unprocessed component HTML) — these are correctness defects, not visual work. Development-VPS measurements (≈16 s full, ≈2.5 s no-op, ≈10 MB RSS on g6-standard-1) are in `reports/benchmarks/nift-dev-build.json` and are **not comparable** to the frozen Astro baseline. Do not describe the entire CP6 checkpoint complete while generated route families remain absent. CP7 may be developed independently; CP8 full parity must not begin from an uncertified corpus (certified = CP6B generated families reconciled and rendering clean).
