...
**CP6 status as of 2026-09-18 (development VPS):** the ordinary-documentation corpus gate has **PASSED**. All 6,882 frozen docs import with zero unknown/unresolved constructs; Nift builds all 6,883 tracked pages; route verifier reports 0 missing routes; `~/assets` references rewritten and resolve. Importer gained corpus-driven robustness (fences, blockquotes, multi-line imports, `</ Tag>` closes) covered by 10 regression tests. Generated/data-driven families (API reference, changelog, glossary, directory, workers-ai models, llms.txt, etc.) are **explicitly recorded** in `reports/CP6-IMPORT.md` with their data sources and link counts — not silently omitted — but are **not yet generated**; they are CP6 follow-on work. Development-VPS build measurements (≈16 s full, ≈2.5 s no-op, ≈10 MB RSS on g6-standard-1) are in `reports/benchmarks/nift-dev-build.json` and are **not comparable** to the frozen Astro baseline. CP7 may be developed independently; CP8 full parity must not begin from an uncertified corpus (certified = generated families reconciled).
