...
**CP6 status as of 2026-09-19 (development VPS):** CP6 is split into **CP6A (ordinary documentation corpus — CERTIFIED)** and **CP6B (complete frozen-site surface + rendering correctness — PENDING)**. CP6A: all 6,882 frozen docs import with zero unknown/unresolved constructs; route verifier reports 0 missing routes; `~/assets`/`src/assets`/`public/` references rewritten; importer corpus robustness covered by 12 regression tests. CP6B so far: generated families (changelog, glossary, directory, fields catalog, Workers AI + catalog models, learning paths, llms.txt) raise the tracked/build surface to **8,631 pages**, all built (~17-19 s full, ~2.5 s no-op on g6-standard-1; development observations only, not comparable to the Astro baseline); titled directives and unclosed directives handled (literal `:::` leakage 1,195 → 0); post-build `tools/leak_scan.py` classifies REAL leakage vs INTENTIONAL code/prose. **Remaining CP6B work:** markdown inside component HTML bodies (the ~97 REAL leakage instances + ~3,500 intentional `**bold**`/backtick text inside component bodies) is a correctness defect; `reports/CP6B-RENDERING-ARCHITECTURE.md` analyses options and recommends the importer emitting Nift `@markup("md"){...}` bodies (verified working, not yet implemented — awaits review). Remaining generated families (compatibility flags, WARP/release, agent-setup, videos, RSS/sitemap, logpush) are recorded with data-source classification; the API-reference family is an external sibling app, not generated. Do not describe the entire CP6 checkpoint complete while generated route families or rendering leakage remain. CP7 may be developed independently; CP8 full parity must not begin from an uncertified corpus (certified = CP6B generated families reconciled and rendering clean).
